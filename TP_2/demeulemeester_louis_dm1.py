...
# 3. M[0] est un entier positif. La suite (S) est une suite récurrente: S(n+1) = S(n) - plus_grand(S(n),
# M) où plus_grand(S(n), M) renvoie un entier positif strictement, donc la suite est décroissante. Donc il existe un
# rang n0 (i.e. un tour de boucle) tel que S(n0) <= M[0] ce qui prouve la terminaison de la fonction.
...
# 5. On peut prendre M = [1, 6, 8] et S = 12. L'algorithme glouton donne [8, 1, 1, 1, 1] alors que la solution
# optimale est [6, 6].
